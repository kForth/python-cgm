"""Hotspot extraction helpers for APD metadata and APS geometry fallback."""
...
from .common import (
    _decode_application_property,
    _decode_hotspot_region_bbox,
    _decode_point_pairs_exact,
    _decode_prefixed_ascii,
...
log = logging.getLogger("cgm.hotspots")


def extract_hotspots_from_bytes(data: bytes) -> list[HotSpot]:
    """Extract hotspot regions from APD metadata and APS geometry fallback."""
    ...
    for element in iter_elements(data):
        ...
        if element.class_id == 4 and context_stack:
            ...
            if element.element_id in (1, 7):
                for point_x, point_y in _decode_point_pairs_exact(element.parameters):
                    _update_geom_bbox(context, point_x, point_y)
            elif element.element_id == 5 and len(element.parameters) >= 8:
                x: float | None = _parse_f32_be(element.parameters[0:4])
                y: float | None = _parse_f32_be(element.parameters[4:8])
