...
def test_extract_final_image_and_hotspots_rejects_image_format_kwarg(tmp_path: Path) -> None:
    ...
    with pytest.raises(TypeError):
        extract_final_image_and_hotspots(src, image_format="png")
...
def test_extract_hotspots_from_gr_77775_contains_autoid_regions() -> None:
    ...
    source = Path(__file__).resolve().parents[1] / "sample" / "GR-77775.cgm"
...
def test_score_bitmap_is_polarity_agnostic() -> None:
    ...
    score = extract_module._score_bitmap(bits, width, height)
    inverted_score = extract_module._score_bitmap(inverted, width, height)
...
def test_score_bitmap_rejects_nearly_single_color_regardless_of_polarity() -> None:
    ...
    black_score = extract_module._score_bitmap(mostly_black, width, height)
    white_score = extract_module._score_bitmap(mostly_white, width, height)
